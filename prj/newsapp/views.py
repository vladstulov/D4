...
class NewsList(ListView):
    ...
    context_object_name = 'news'  # имя списка, в котором будут лежать все объекты, его следует указать, чтобы обратиться к самому списку объектов через HTML-шаблон
...
class NewsSearchList(ListView):
    model = News  # указываем модель, объекты которой мы будем выводить
    template_name = 'newsSearch.html'  # указываем имя шаблона, в котором будет лежать HTML
...
class NewsDetail(DetailView):
    ...
    template_name = 'note.html'  # название шаблона будет note.html
